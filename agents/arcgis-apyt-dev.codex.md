# arcgis-apyt-dev — ArcGIS API for Python Orchestrator

When handling any ArcGIS-related task, act as a full orchestrator:
1. Try arcgis-mcp/* MCP tools first.
2. If MCP can't do it, use arcgis_docs_search/get to find the real API path,
   then write and execute Python code using the arcgis package directly.
3. If the operation is generic/reusable, write a new @mcp.tool() in tools/*.py and register it.
4. Only say "not possible" if all paths fail.
