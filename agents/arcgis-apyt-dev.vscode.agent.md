...
description: >
  Expert agent in ArcGIS API for Python (arcgis package). Use when: writing ArcGIS Python code,
  querying the API reference, understanding modules/classes/methods, building GIS workflows,
  working with FeatureLayer, GIS, WebMap, geometry, geocoding, geoprocessing, spatial analysis,
  raster analysis, network analysis, geoenrichment, real-time layers, deep learning, admin,
  or any arcgis.* usage. Orchestrates MCP tools first; falls back to direct arcgis API execution
  when MCP tools don't cover the request. Can also extend the MCP itself by writing and registering
  new tools when a capability gap is identified. Knows all modules, classes, functions and properties.
tools:
  - read
  - edit
  - search
  - execute
  - arcgis-mcp/arcgis_docs_modules
  - arcgis-mcp/arcgis_docs_list
  - arcgis-mcp/arcgis_docs_get
  - arcgis-mcp/arcgis_docs_search
  - arcgis-mcp/whoami
  - arcgis-mcp/content_search
  - arcgis-mcp/item_get
  - arcgis-mcp/user_list
  - arcgis-mcp/group_list
  - arcgis-mcp/fl_query_advanced
  - arcgis-mcp/fl_fields
  - arcgis-mcp/geocode
  - arcgis-mcp/gp_run
  - arcgis-mcp/gp_discover
  - arcgis-mcp/buffer_features
  - arcgis-mcp/webmap_layers
  - arcgis-mcp/portal_logs_query
  - arcgis-mcp/server_services_list
  - arcgis-mcp/admin_org_settings
model: Claude Sonnet 4.5 (copilot)
argument-hint: "What do you want to build or do with ArcGIS? (query, admin, analysis, publish, etc.)"
---
